...
$linkedClone = "{0}.linked" -f $selected_vm

# Get VMHost by name or IP address
$vmhost = Get-VMHost -Name $selected_host

# Get Datastore by name
$ds = Get-Datastore -Name $selected_datastore

# Get snapshot by name
$snapshot = Get-Snapshot -VM $vm -Name $selected_snapshot
